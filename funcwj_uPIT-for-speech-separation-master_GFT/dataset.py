...
import utils
from utils import parse_scps, stft, apply_cmvn, EPSILON, get_logger, stft_GFT
...
class SpectrogramReader(object):
    ...
    #def _load(self, key):
        #return stft_GFT(self.wave_dict[key], **self.stft_kwargs)
...
class DataLoader(object):
    ...
    def _transform(self, mixture_specs, targets_specs_list):
        ...
        mixture_stft, mixture_gft = mixture_specs.split([129, 256], 1)

        inputnum = mixture_specs.shape[1]
        outputnum = mixture_stft.shape[1]
        mixture_specs = th.unsqueeze(mixture_specs, 2)

        gft_stft_out = nn.Conv1d(inputnum, outputnum, 1, bias=False)
        stft_gft_mat1 = gft_stft_out(mixture_specs)
        stft_gft_mat2 = th.squeeze(stft_gft_mat1, 2)
        mixture_specs = stft_gft_mat2.detach().numpy()
        ...
        if np.iscomplexobj(mixture_specs):
            ...
            source_attr["spectrogram"] = th.tensor(
                np.abs(mixture_stft), dtype=th.float32)

           # source_attr["stft_spectrogram"] = th.tensor(
                #np.abs(mixture_stft), dtype=th.float32)
            ...
            source_attr["phase"] = th.tensor(
                np.angle(mixture_stft), dtype=th.float32)
    ...
    def _process(self, index):
        ...
        source_attr["spectrogram"] = pad_sequence(
            ...
            batch_first=True)
        #source_attr["stft_spectrogram"] = pad_sequence(
            #[d["source_attr"]["stft_spectrogram"] for d in dict_list],
           #batch_first=True)
        target_attr["spectrogram"] = [
